...
gint compare_user_infoF(const void *a, const void *b)
{
    USER_INFO_F *user_info_a = (USER_INFO_F *)a;
    USER_INFO_F *user_info_b = (USER_INFO_F *)b;
...
void query9F(USERS_CATALOG catalog, char *args, char *output)
{
    ...
    GArray *userArray = g_array_new(FALSE, FALSE, sizeof(USER_INFO_F));

    iter_hash_query9(catalog, userArray, args);

    if (userArray->len > 0) {
        qsort(userArray->data, userArray->len, sizeof(USER_INFO_F), compare_user_infoF);
    }

    for (guint i = 0; i < userArray->len; i++)
    {
        USER_INFO_F info = g_array_index(userArray, USER_INFO_F, i);
        if ((userArray->len) - 1 == i)
            fprintf(outputFile, "--- %d ---\nid: %s\nname: %s\n", i + 1, info.user_id, info.username);
        else
            fprintf(outputFile, "--- %d ---\nid: %s\nname: %s\n\n", i + 1, info.user_id, info.username);
    }
